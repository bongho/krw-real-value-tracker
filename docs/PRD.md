...
#### 3.1.0 데이터 메타정보 배너 (v2.5.1)

**위치**: 헤더 바로 아래, 모든 차트/컴포넌트 위

**목적**: 사용자에게 데이터의 신뢰성과 최신성을 명확히 전달

**표시 정보**:

| 항목 | 아이콘 | 내용 | 포맷 |
|------|--------|------|------|
| **최종 업데이트** | 📅 | 데이터 마지막 갱신 시각 | YYYY년 MM월 DD일 HH:MM |
| | | 경과 시간 | X시간 전 |
| **업데이트 주기** | 🔄 | 데이터 갱신 주기 | 일 1회 (매일 자정) |
| | | 다음 업데이트 예정 시각 | 내일 00:00 |
| **기준 시점** | 📍 | M2 계산 기준 날짜 | YYYY년 MM월 DD일 |
| | | 기준 환율 | 1,167원 |
| **데이터 포인트** | 📊 | 시계열 데이터 개수 | N개 시계열 |
| **업데이트 상태** | 🟢/🟡 | 실시간 상태 표시 | 최신 데이터 / 업데이트 대기 중 |

**상태 표시 로직**:
- **최신 데이터** (🟢 녹색): 마지막 업데이트 후 24시간 이내
- **업데이트 대기 중** (🟡 노란색): 마지막 업데이트 후 24시간 이상
...
{dataset && (
  <div className="metadata-banner">
    <div>📅 최종 업데이트: 2025년 11월 16일 18:26 (9시간 전)</div>
    <div>🔄 업데이트 주기: 일 1회 (매일 자정) | 다음: 내일 00:00</div>
    <div>📍 기준 시점: 2010년 1월 1일 | 기준환율: 1,167원</div>
    <div>📊 데이터 포인트: 1,825개 시계열</div>
    <div>🟢 최신 데이터</div>
  </div>
)}
```
...
**사용자 가치**:
- 데이터 신뢰성 확보 (언제 마지막으로 업데이트되었는지 즉시 확인)
- 다음 업데이트 예상 시각 제공
- M2 계산의 기준점 명확화
- 데이터 양에 대한 투명성 제공
...
### Q5: 왜 1개월, 3개월 같은 단기 조회 옵션이 없나요?
**A**: M2 통화량 데이터는 **월별로만 발표**되기 때문에, 일별 또는 주별 단위로 의미 있는 분석이 불가능합니다.

**데이터별 업데이트 주기**:

| 데이터 | 업데이트 주기 | 출처 | 비고 |
|--------|-------------|------|------|
| **한국 M2** | 월 1회 (익월 중순) | ECOS (한국은행) | 전월 데이터 |
| **미국 M2** | 월 1회 (익월 초) | FRED (세인트루이스 연은) | 전월 데이터 |
| **시장 환율** | 일 1회 | ExchangeRate-API | 실시간 가능하나 M2 월별 특성상 의미 없음 |
| **금리 (한국/미국)** | 월 1회 | FRED | 정책 금리는 월 1-2회 변경 |
| **VIX** | 일 1회 | FRED | 시장 거래일 기준 |
| **DXY** | 일 1회 | FRED | 시장 거래일 기준 |
| **경상수지** | 분기 1회 | FRED | 분기 종료 후 1-2개월 뒤 발표 |
| **GDP** | 분기 1회 | FRED | 분기 종료 후 1-2개월 뒤 발표 |
| **CPI** | 월 1회 (익월 초) | FRED | 전월 데이터 |

**왜 1년 미만 조회가 의미 없는가?**
- M2 괴리율은 **통화량 증가율의 장기 추세**를 기반으로 계산됩니다.
- 월별 데이터 특성상 **최소 12개 데이터 포인트** (1년)가 있어야 추세를 파악할 수 있습니다.
- 1-3개월 같은 초단기 변동은 **노이즈에 가까우며** 투자 의사결정에 오히려 해롭습니다.

**권장 조회 기간**:
- **최소**: 1년 (12개 데이터 포인트)
- **권장**: 3년 (경기 사이클 1회분)
- **장기 분석**: 5년 또는 전체 (2010년~)
